...
本模块提供了RuYuan MQ的完整使用示例，涵盖了从基础功能到高级特性的各种使用场景。
...
**运行方式：**
```bash
java -cp target/classes com.ruyuan.mq.example.quickstart.QuickStartExample
```
...
**运行方式：**
```bash
java -cp target/classes com.ruyuan.mq.example.producer.SimpleProducerExample
```
...
**运行方式：**
```bash
java -cp target/classes com.ruyuan.mq.example.consumer.SimpleConsumerExample
```
...
**运行方式：**
```bash
java -cp target/classes com.ruyuan.mq.example.advanced.AdvancedFeaturesExample
```
...
**运行方式：**
```bash
java -cp target/classes com.ruyuan.mq.example.performance.PerformanceTestExample
```
...
**运行方式：**
```bash
java -cp target/classes com.ruyuan.mq.example.cluster.ClusterExample
```
...
```bash
# 启动NameServer
java -cp ruyuan-mq-nameserver.jar com.ruyuan.mq.nameserver.NameServerStartup
```
...
```bash
# 启动Broker
java -cp ruyuan-mq-broker.jar com.ruyuan.mq.broker.BrokerStartup
```
...
```bash
# 在项目根目录执行
mvn clean compile -pl ruyuan-mq-example
```
