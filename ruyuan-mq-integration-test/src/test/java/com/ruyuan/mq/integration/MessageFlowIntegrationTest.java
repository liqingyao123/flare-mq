package com.ruyuan.mq.integration;
...
import com.ruyuan.mq.broker.ack.AckResult;
import com.ruyuan.mq.broker.ack.AckStatus;
import com.ruyuan.mq.broker.ack.AckStats;
...
import com.ruyuan.mq.client.consumer.PullResult;
...
class MessageFlowIntegrationTest {
    ...
    void testMessageRetryFlow() {
        ...
        // 5. 验证重试记录
        java.util.List<com.ruyuan.mq.broker.ack.RetryRecord> retryMessages = ackManager.getAllRetryMessages();
        assertEquals(1, retryMessages.size(), "应该有1个重试消息");

        com.ruyuan.mq.broker.ack.RetryRecord retryRecord = retryMessages.get(0);
    ...
    void testDeadLetterQueueFlow() {
        ...
        // 6. 验证死信记录
        java.util.List<com.ruyuan.mq.broker.ack.DeadLetterRecord> deadLetterMessages = ackManager.getDeadLetterMessages();
        assertEquals(1, deadLetterMessages.size(), "应该有1个死信消息");

        com.ruyuan.mq.broker.ack.DeadLetterRecord deadLetterRecord = deadLetterMessages.get(0);
    ...
    void testTopicQueueManagementIntegration() {
        // 1. 记录初始状态
        com.ruyuan.mq.broker.topic.TopicStats initialTopicStats = topicManager.getTopicStats();
        com.ruyuan.mq.broker.queue.QueueStats initialQueueStats = queueManager.getQueueStats();
        ...
        // 3. 验证Topic统计（验证增量）
        com.ruyuan.mq.broker.topic.TopicStats topicStats = topicManager.getTopicStats();
        ...
        // 4. 验证Queue统计
        com.ruyuan.mq.broker.queue.QueueStats queueStats = queueManager.getQueueStats();
        ...
        // 6. 验证删除后的统计（验证删除操作成功）
        com.ruyuan.mq.broker.topic.TopicStats finalTopicStats = topicManager.getTopicStats();
        ...
        com.ruyuan.mq.broker.queue.QueueStats finalQueueStats = queueManager.getQueueStats();
    ...
    void testProducerConsumerConfiguration() {
        ...
        java.util.Map<String, com.ruyuan.mq.client.consumer.SubscriptionData> subscriptions = consumer.getSubscriptions();
        ...
        java.util.Map<String, com.ruyuan.mq.client.consumer.SubscriptionData> emptySubscriptions = consumer.getSubscriptions();
    ...
    void testSystemHealthCheck() {
        ...
        com.ruyuan.mq.broker.ack.AckStats.HealthStatus healthStatus = ackStats.getHealthStatus();
        assertTrue(healthStatus == com.ruyuan.mq.broker.ack.AckStats.HealthStatus.WARNING ||
                  healthStatus == com.ruyuan.mq.broker.ack.AckStats.HealthStatus.CRITICAL,
